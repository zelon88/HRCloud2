...
class UberGallery {
    ...
    public function __construct() {
      global $UserID, $InstLoc;
        ...
        $this->_appDir = $InstLoc.'/DATA/'.$UserID.'/.AppData/UberGallery';

        // Set configuration file path
        $configPath = $this->_appDir . '/resources/galleryConfig.ini';
        ...
        if (file_exists($configPath)) {
            ...
            $this->setCacheDirectory($this->_appDir . '/resources/cache');
    ...
    public function createGallery($directory, $relText = 'colorbox') {
        ...
        $templatePath = $this->_appDir . '/resources/templates/defaultGallery.php';
        ...
        if (!$galleryArray) {
            ...
            foreach ($dirArray as $key => $image) {
                ...
                $galleryArray['images'][htmlentities(pathinfo($image['real_path'], PATHINFO_BASENAME))] = array(
                    ...
                    'file_path'    => str_replace('/var/www/html', '', htmlentities($relativePath)),
    ...
    public function getThemePath($absolute = false) {
        if ($absolute) {
            // Set the theme path
            $themePath = $this->_appDir . '/resources/themes/' . $this->_config['theme_name'];
        } else {
            // Get relative path to application dir
            $realtivePath = $this->_getRelativePath(getcwd(), $this->_appDir);

            // Set the theme path
            $themePath = $realtivePath . '/resources/themes/' . $this->_config['theme_name'];
        }
    ...
    public function getColorboxStyles($themeNum) {
        ...
        // Set ColorBox path
        $colorboxPath = $realtivePath . '/resources/colorbox/' . $themeNum . '/colorbox.css';
    ...
    public function getColorboxScripts() {
        ...
        $templatePath = $this->_appDir . '/resources/templates/colorboxScripts.php';
        $colorboxPath = $this->_getRelativePath(getcwd(), $this->_appDir) . '/resources/colorbox/jquery.colorbox.js';
